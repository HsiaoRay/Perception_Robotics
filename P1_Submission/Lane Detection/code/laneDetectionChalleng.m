...
close all;
clc;
video = VideoReader('challenge_video.mp4');
output = VideoWriter('challenge_output1','MPEG-4');
...
while hasFrame(video)
  ...
  [H,T,R] = hough(roi);
  P = houghpeaks(H,2);
  lines = houghlines(mask,T,R,P,'FillGap',100,'MinLength',10);
  ...
  if (~isnan(x1) && abs(x(1)-x1) < 30)          
      x(1) = x1; 
  end
  if (~isnan(x2) && abs(x(2)-x2) < 30)
      x(2) = x2; 
  end
  if (~isnan(x3) && abs(x(3)-x3) < 30)
      x(3) = x3; 
  end
  if (~isnan(x4) && abs(x(4)-x4) < 30)
      x(4) = x4; 
  end
  y(1) = y1; 
  y(2) = y2; 
